...
SIM_CONFIG: mosaik.SimConfig = {
    ...
    "ATPolicy": {"python": "simulator_curtailment_policy.at_nolimit_policy_sim:ATNoLimitPolicySim"},
...
with mosaik.World(SIM_CONFIG) as world:
    ...
    atpol = world.start("ATPolicy", step_size=STEP)
    ctrl = world.start("Ctrl", step_size=STEP, country="AT")

    grid = pp.Grid(simbench="1-LV-rural2--0-sw")
    hdf5 = db.Database(
        filename="simresults_ex_scenario_austriacontroller.hdf5")
    ...
    for li in controllable_load_idxs:
        ...
        # 1) DSO Policy (unlimited)
        pol = atpol.ATNoLimitPolicy()
        ...
        # 5) No-Limit kommt von der Policy
    ...
    # mosaik.util.connect_many_to_one(
    #     world, load_entities, hdf5, "P[MW]", "Q[MVar]")
    mosaik.util.connect_many_to_one(
