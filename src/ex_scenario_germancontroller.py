...
SIM_CONFIG: mosaik.SimConfig = {
    ...
    # "EVProfile": {"python": "controller_grid.ev_charging_profile_sim:EVChargingProfileSim"},
    "Policy": {"python": "simulator_curtailment_policy.policy_sim:PolicySim"},
    "State": {"python": "simulator_curtailment_policy.state_schedule_sim:StateScheduleSim"},
    "DEPolicy": {"python": "simulator_curtailment_policy.de_direct_control_sim:DEDirectControlPolicySim"},
...
# THRESH_MW = 4.2 / 1000.0  # controllable loads
...
with mosaik.World(SIM_CONFIG) as world:
    ...
    # evsim = world.start("EVProfile", step_size=STEP)
    polsim = world.start("Policy", step_size=STEP)
    statesim = world.start("State", step_size=STEP)
    depol = world.start("DEPolicy", step_size=STEP)
    ctrl = world.start("Ctrl", step_size=STEP, country="DE")

    grid = pp.Grid(simbench="1-LV-rural2--0-sw")
    hdf5 = db.Database(filename="simresults_ex_scenario_germancontroller.hdf5")

    # Policy (global/shared)
    policy = polsim.ContractPolicy(curtailment_cmd=1)

    state = statesim.StateSchedule(
        state_normal=0,
        state_event=3,
        event_start_h=17.25,
        event_end_h=20.25,
    ...
    # Loads, die >4.2kW sind
    # controllable_load_idxs = [int(i) for i in net.load.index if float(net.load.at[int(i), "p_mw"]) > THRESH_MW]
    # alle Loads werden Controllable Loads
    controllable_load_idxs = [int(i)
                              for i in net.load.index if int(i) in load_by_idx] # type: ignore
    ...
    for li in controllable_load_idxs:
        ...
        # 1) DSO Policy (kW Limit)
        pol = depol.DEDirectControlPolicy(
            min_power_kw=4.2,
            default_limit_kw=rated_mw * 1000.0,   # default: keine Begrenzung
        ...
        # --- gewünschte Leistung ---
        # ev = evsim.EVChargingProfile(
        #     rated_power_kw=11.0,
        #     start_hour=18.0,
        #     end_hour=6.5,
        #     energy_kwh=20.0,
        #     seed=li,              # li = load index (damit reproduzierbar)
        #     jitter_minutes=45.0,
        #     ramp_minutes=5.0,
        # )
        # world.connect(ev, dev, ("p_request_mw", "p_request_mw"))

        # 4) Sollwert kommt aus ursprünglichem Load-Profil (P[MW])
        src_load = load_by_idx[li]
        world.connect(src_load, dev, ("P[MW]", "p_request_mw"))
        world.connect(src_load, pol, ("P[MW]", "p_request_mw"))   # optional (ignored)
    ...
    # EV Wunschleistung
    # mosaik.util.connect_many_to_one(world, ev_entities, hdf5, "p_request_mw")
    ...
    world.connect(state, hdf5, "control_state")
